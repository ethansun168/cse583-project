#!/bin/bash
# Run script for Dynamic Analysis in Final Project EECS 583 Winter 2024
...
PATH2LIB="./build/DynamicPass/DynamicPass.so"
...
for bench in input/*.c input/*.cpp; do
    ...
    opt -disable-output -load-pass-plugin="$PATH2LIB" -passes="dynamic-analysis" "${base}.ll" 2> "${base}.txt"
    if [ $? -ne 0 ]; then
